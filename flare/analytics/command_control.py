...
from multiprocessing import Process, JoinableQueue, Lock, Manager,set_start_method, Queue
...
config_default = os.path.join(os.path.dirname(
    __file__), '..', '..', 'configs/elasticsearch.ini')
...
class elasticBeacon(object):
    ...
    def __init__(self,
        ...
        if config_in is not None:
            try:
                ...
                self.es_use_ssl = self.config.config.getboolean(
                    'beacon', 'es_use_ssl')
                self.es_ca_certs = self.config.get('beacon', 'es_ca_certs')
                self.es_verify_certs = self.config.config.getboolean(
                    'beacon', 'es_verify_certs')
                self.MIN_OCCURRENCES = int(
                    self.config.get('beacon', 'min_occur'))
                self.MIN_PERCENT = int(
                    self.config.get('beacon', 'min_percent'))
                ...
                self.kibana_version = self.config.get(
                    'beacon', 'kibana_version')
                self.beacon_src_ip = self.config.get(
                    'beacon', 'field_source_ip')
                self.beacon_dest_ip = self.config.get(
                    'beacon', 'field_destination_ip')
                ...
                self.beacon_flow_id = self.config.get(
                    'beacon', 'field_flow_id')
                ...
                qstart_config = self.config.get('beacon','querystart')
                self.query_start = time.time() if qstart_config.strip()== '' else time.mktime(datetime.strptime(qstart_config,'%d/%m/%Y %H:%M:%S').timetuple())
                self.whitelist = self.config.config.items("whitelist")
                try:
                ...
                print(('{red}[FAIL]{endc} Could not properly load your config!\nReason: {e}'.format(
                    red=bcolors.FAIL, endc=bcolors.ENDC, e=e)))
                sys.exit(0)
            ...
            self.es_ca_certs = es_ca_certs
            self.es_verify_certs = es_verify_certs
            self.es_use_ssl = es_use_ssl
        ...
        self.info = '{info}[INFO]{endc}'.format(
            info=bcolors.OKBLUE, endc=bcolors.ENDC)
        self.success = '{green}[SUCCESS]{endc}'.format(
            green=bcolors.OKGREEN, endc=bcolors.ENDC)
        self.fields = [self.beacon_src_ip, self.beacon_dest_ip, self.beacon_destination_port,
                       'bytes_toserver', 'mean_bytes_toserver','median_bytes_toserver','mode_bytes_toserver', 'dest_degree', 'occurrences', 'percent', 'interval']
    ...
    def split_duration(self, start, end, count):
        durations=[]
        step = int((start - end)/count )
        pos = start
        while pos > end:
            lte = pos
            pos -= step
            gt = pos
            if pos <= end:
                durations.append({"gte":gt, "lte": lte, "format": "epoch_millis"})
            else:
                durations.append({"gt":gt, "lte": lte, "format": "epoch_millis"})
        return durations 
    def get_query(self, duration_range, fields):
        if self.es_index:
            query = {
                "query": {
                    self.filt: {
                        ...
                        "filter": [{
                            "bool": {
                                "must": [
                                    {
                                        "range": {
                                            self.beacon_timestamp: duration_range
                                        }
                                    },
                                ],
                                "must_not": [
                                    {
                                        "terms":{
                                            self.beacon_dest_ip:  [item[1] for item in self.whitelist]
                                        } 
                                    }
                                ]
                            }
        ...
        if fields:
            ...
            self.dprint(query)
        return query
    def hour_queries(self, h, *fields):
        """
        ...
        NOW = int(self.query_start * 1000)
        SECONDS = 1000
        MINUTES = 60 * SECONDS
        HOURS = 60 * MINUTES
        END = int(self.query_start * 1000 - h * HOURS )
        durations = self.split_duration(NOW,END,self.NUM_PROCESSES)
        queries = map(lambda duration_range: self.get_query(duration_range, fields),durations)
        return list(queries)
    ...
    def process_query(self,query,queue):
        def worker_print(message):
            self.vprint("{green}[Worker: {pid}]{endc} {message}".format(green=bcolors.OKGREEN, endc= bcolors.ENDC, message=message, pid=os.getpid()))
        worker_print("my query is {range}".format(range=query))
        try:
            self.vprint('{info}[INFO]{endc} Attempting to connect to elasticsearch...'.format(
                info=bcolors.OKBLUE, endc=bcolors.ENDC))
            ...
            if self.auth == "None":
                es = Elasticsearch(
                    self.es_host, port=self.es_port, timeout=self.es_timeout, use_ssl=self.es_use_ssl, ssl_context=ssl_context)
            else:
                es = Elasticsearch(self.es_host, port=self.es_port, timeout=self.es_timeout, http_auth=(
                    self.auth_user, self.auth_password), use_ssl=self.es_use_ssl, ssl_context=ssl_context)
            self.vprint('{green}[SUCCESS]{endc} Connected to elasticsearch on {host}:{port}'.format(
                green=bcolors.OKGREEN, endc=bcolors.ENDC, host=self.es_host, port=str(self.es_port)))
        except Exception as e:
            self.vprint(e)
            raise Exception(
                "Could not connect to ElasticSearch -- Please verify your settings are correct and try again.")
        resp = helpers.scan(query=query, client=es,
                            scroll="10m", index=self.es_index, timeout="10m", size=10000,)
        worker_print("got response")
        worker_print("extracting data")
        data= [r['_source'] for r in resp]
        worker_print("got data")
        queue.put(data)
        worker_print("exiting")



    def run_query(self):
        self.vprint(
            "{info} Gathering flow data... this may take a while...".format(info=self.info))

        FLOW_BYTES = self.beacon_flow_bytes_toserver
        if self.suricata_defaults:
            FLOW_BYTES = 'flow.' + FLOW_BYTES

        queries = self.hour_queries(self.period, self.beacon_src_ip, self.beacon_dest_ip, self.beacon_destination_port,
        ...
        resp = []
        #creating processes
        processes = [Process(target=self.process_query, args=(query, queue), name="beacon_detector_worker", daemon=True) for query in queries]
        #starting processes
        for process in processes:
            process.start()
        master_state = 0 
        while len(list(filter(lambda p: p.is_alive(), processes))) > 0 or not queue.empty():
            if queue.empty():
                time.sleep(1.0)
                if master_state == 0:
                    print("[Master] waiting for data. State: %d"%master_state)    
                master_state += 1
                continue
            print("[Master] ready for next data")
            data = queue.get()
            print("[Master] got some data. Size: %d records" %len(data))
            resp.extend(data)
            master_state = 0
        print("[Master] have all data")
        print("[Master] joining all processes")
        ...
        print("about to call dataframe")
        df = pd.DataFrame(resp)
        print("finished calling dataframe")
        if len(df) == 0:
            raise Exception(
                "Elasticsearch did not retrieve any data. Please ensure your settings are correct inside the config file.")

        self.dprint(df)
        df['dest_port'] = df[self.beacon_destination_port].fillna(
            0).astype(int)
        
        if 'flow' in df.columns:
            df[self.beacon_flow_bytes_toserver] = df['flow'].apply(
                lambda x: x.get(self.beacon_flow_bytes_toserver))
        else :
            df[self.beacon_flow_bytes_toserver] = df[self.beacon_flow_bytes_toserver].fillna(0).astype(int)

        df['triad_id'] = (df[self.beacon_src_ip] + df[self.beacon_dest_ip] +
        ...
        self.high_freq = list(
            df[df.triad_freq > self.MIN_OCCURRENCES].groupby('triad_id').groups.keys())
        return df
    ...
    def find_beacon(self, q_job, beacon_list):
        while not q_job.empty():
            ...
            work[self.beacon_timestamp] = (
                work[self.beacon_timestamp].astype(int) / 1000000000).astype(int)
            ...
            if d and total > self.MIN_OCCURRENCES:
                ...
                if percent > self.MIN_PERCENT and total > self.MIN_OCCURRENCES:
                    ...
                    MEAN_BYTES_TOSERVER = work[self.beacon_flow_bytes_toserver].mean()
                    MEDIAN_BYTES_TOSERVER = work[self.beacon_flow_bytes_toserver].median()
                    MODE_BYTES_TOSERVER = tuple(work[self.beacon_flow_bytes_toserver].mode().to_list())
                    SRC_DEGREE = len(work[self.beacon_dest_ip].unique())
                    OCCURRENCES = total
                    self.l_list.acquire()
                    beacon_list.append(
                        [SRC_IP, DEST_IP, DEST_PORT, BYTES_TOSERVER,MEAN_BYTES_TOSERVER,MEDIAN_BYTES_TOSERVER,MODE_BYTES_TOSERVER, SRC_DEGREE, OCCURRENCES, PERCENT, WINDOW])
                    self.l_list.release()
    ...
    def find_beacons(self, group=True, focus_outbound=False, whois=True, csv_out=None, html_out=None, json_out=None):
        ...
        if whois:
            self.vprint(
                '{info} Enriching IP addresses with whois information'.format(info=self.info))
        ...
        if focus_outbound:
            self.vprint(
                '{info} Applying outbound focus - filtering multicast, reserved, and private IP space'.format(info=self.info))
        ...
        if group:
            self.vprint(
                '{info} Grouping by destination group IP'.format(info=self.info))

            if whois:
                self.fields.insert(self.fields.index(
                    self.beacon_dest_ip), 'dest_whois')
        ...
        if json_out:
            ...
            if group:
                table = pd.DataFrame(beacon_df.index.tolist() ,columns=self.fields)
                table.to_json(json_out,orient="records")
            else:
